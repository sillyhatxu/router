...
func testCreatedAPI(context *gin.Context)  {
	...
	if err := context.ShouldBindJSON(&requestBody); err == nil {
		context.JSON(http.StatusOK, http_response.Success("AAAAA----- Created : {Name:" + requestBody.Name+ ",Age:" + strconv.Itoa(requestBody.Age) + "}"))
	} else {
...
func testUpdatedAPI(context *gin.Context)  {
	...
	var requestBody *User
	if err := context.ShouldBindJSON(&requestBody); err == nil {
		context.JSON(http.StatusOK, http_response.Success("AAAAA----- Updated : {id:" + id + ",Name:" + requestBody.Name+ ",Age:" + strconv.Itoa(requestBody.Age) + "}"))
	} else {
		context.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
	}
}
...
func testGetAPI(context *gin.Context)  {
	id := context.Param("id")
	context.JSON(http.StatusOK, http_response.Success("AAAAA----- Get : "+id))
}

func testDeletedAPI(context *gin.Context)  {
	id := context.Param("id")
	context.JSON(http.StatusOK, http_response.Success("AAAAA----- Deleted : "+id))
}
